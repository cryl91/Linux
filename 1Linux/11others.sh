...
#     3. zip/unzip → good for Windows compatibility and tar/tar.gz → better for Linux backups (permissions, symlinks, performance).
...
#     2. tar -xzf archive.tar.gz /path/to/extract = x is to extract
...
mkdir "$DEST_DIR"
# Timestamp
TIMESTAMP=$(date +"%Y%m%d_%H%M%S") # O/P is 20250820_232909
