...
active
--[ec2-user@ip-172-31-20-206 ~]$ sudo systemctl is-active docker --quiet #Doesnt give the "active" word output like before line.
--[ec2-user@ip-172-31-20-206 ~]$ sudo systemctl is-active docker
...
--In Linux shell scripting, && is a logical AND operator. It means, Run the command on the right only if the command on the left succeeds (exit status = 0).
...
--If you want a code to run automatically, you can add it to cron = 
    1. Open the Crontab Editor = crontab -e
    2. Syntax = * * * * * command-to-run++++Ex = 0 2 * * * /usr/local/bin/logrotate.sh
    3. Syntax = Minute Hour Dayofmonth Month Dayofweek
...
--How to automate the transfer of files from a local machine to a remote server using scp(secure copy) = 
    ...
    2. To copy a directory: scp -r /path/to/local/dir username@remotehostIP:/path/to/remote/directory/
...
--Reboot the system = 
    1. sudo reboot = it restarts the entire machine (affecting all users)
    2. pkill -KILL -u <username> = If you only want to restart a particular user. This kills all processes of that user (effectively "rebooting" their session).
